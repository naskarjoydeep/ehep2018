...
int main(int argc, char* argv[]) {
  ...
  TApplication theApp("WWjets", &argc, argv);
  ...
  double radius   = 0.7;
  double pTjetMin = 10.;
  // Exclude neutrinos (and other invisible) from stuWW.
  int    nSel     = 2;
 ...
 // Create file on which histogram(s) can be saved.
  TFile* outFile1 = new TFile("WWnJetsS.root", "RECREATE");
  //TFile* outFile2 = new TFile("WWnJetsC.root", "RECREATE");
  //TFile* outFile3 = new TFile("WWnJetsD.root", "RECREATE");
  TFile* outFile4 = new TFile("WWpTJetsS.root", "RECREATE");
  //TFile* outFile5 = new TFile("WWeTJetsC.root", "RECREATE");
  TFile* outFile6 = new TFile("WWyJetsS.root", "RECREATE");
  //TFile* outFile7 = new TFile("WWetaJetsC.root", "RECREATE");
  TFile* outFile8 = new TFile("WWphiJetsS.root", "RECREATE");
  //TFile* outFile9 = new TFile("WWphiJetsC.root", "RECREApTTE");
  TFile* outFile10 = new TFile("WWdistJetsS.root", "RECREATE");
  //TFile* outFile11 = new TFile("WWdistJetsC.root", "RECREATE");
  TFile* outFile12 = new TFile("WWpTdiffS.root", "RECREATE");
  //TFile* outFile13 = new TFile("WWeTdiffC.root", "RECREATE");
  TFile* outFile14 = new TFile("WWnLeptons.root", "RECREATE");
  TFile* outFile15 = new TFile("WWLeptonpT.root", "RECREATE");
  TFile* outFile16 = new TFile("WWLeptonEta.root", "RECREATE");
  TFile* outFile17 = new TFile("WWMET.root", "RECREATE");


// Book histogram.
  TH1F *nJetsS = new TH1F("WWnJetsS","WW number of jets, SlowJet", 50, -0.5, 49.5);
  //TH1F *nJetsC = new TH1F("WWnJetsC","WW number of jets, CellJet", 50, -0.5, 49.5);
  //TH1F *nJetsD = new TH1F("WWnJetsD","WW number of jets, CellJet - SlowJet", 45, -22.5, 22.5);
  TH1F *pTJetsS = new TH1F("WWpTJetsS","WW pT for jets, SlowJet", 100, 0., 250.);
  //TH1F *eTJetsC = new TH1F("WWeTJetsC","WW eT for jets, CellJet", 100, 0., 250.);
  TH1F *yJetsS = new TH1F("WWyJetsS","WW y for jets, SlowJet", 100, -5., 5.);
  //TH1F *etaJetsC = new TH1F("WWyJetsC","WW eta for jets, CellJet", 100, -5., 5.);
  TH1F *phiJetsS = new TH1F("WWphiJetsS","WW phi for jets, SlowJet", 100, -M_PI, M_PI);
  //TH1F *phiJetsC = new TH1F("WWphiJetsC","WW phi for jets, CellJet", 100, -M_PI, M_PI);
  TH1F *distJetsS = new TH1F("WWdistJetsS","WW R distance between jets, SlowJet", 100, 0., 10.);
  //TH1F *distJetsC = new TH1F("WWdistJetsC","WW R distance between jets, CellJet", 100, 0., 10.);
  TH1F *pTdiffS = new TH1F("WWpTdiffS","WW pT difference, SlowJet", 100, -100., 400.);
  //TH1F *eTdiffC = new TH1F("WWeTdiffC","WW eT difference, CellJet", 100, -100., 400.);
  TH1F *nLepton = new TH1F("WWnLepton","WW number of leptons", 20, -0.5, 19.5);
  //TH1F *m0JetsC = new TH1F("WWm0","WW Invariant Mass of Jets, CellJet", 100, 0., 1000.);
  TH1F *pTLepton = new TH1F("WWpTLepton","WW pT of Leptons", 100, 0., 250.);
  TH1F *etaLepton = new TH1F("WWetaLepton","WW eta of Leptons", 20, -10., 10.);
  TH1F *MET = new TH1F("WWMET","WW Missing Energy", 100, 0., 100.);
